...
### Terminal 1 — Start the Pin Config Server
```bash
ros2 run slvrov_nodes_python pca9685_pin_configs_server
```

### Terminal 2 — Start the Pin Config Client (to register servos)
```bash
ros2 run slvrov_nodes_python pca9685_pin_configs_client
```

### Terminal 3 — Start the PCA9685 Node (to control servos)
```bash
...
Once everything is running, you can publish commands to move a servo:

```bash
ros2 topic pub /pca9685_command slvrov_interfaces/msg/PCA9685Command \
  "{id: ['my_servo'], pwm: [0.0]}"
```

The `pwm` value ranges from `-1.0` (full reverse) to `1.0` (full forward), with `0.0` being neutral/center.
